...
export default function DashboardPage() {
  ...
  // Simulate live updates for real-time operations
  useEffect(() => {
    const interval = setInterval(() => {
      if (activeTab === 'operations') {
        setLiveStats(prev => ({
          activeQueue: Math.max(0, prev.activeQueue + Math.floor(Math.random() * 20) - 10),
          activeTransactions: Math.max(0, prev.activeTransactions + Math.floor(Math.random() * 10) - 5),
          blockedBots: Math.max(0, prev.blockedBots + Math.floor(Math.random() * 5) - 2),
          serverLoad: Math.max(30, Math.min(90, prev.serverLoad + Math.floor(Math.random() * 6) - 3))
        }));
      }
    }, 3000);
  ...
  // Generate predictions with mock data (exact copy from simulation)
  ...
  // Generate historical data for selected year (exact copy from simulation)
  ...
  // Auto-generate on component mount and when prediction days change (exact copy from simulation)
  useEffect(() => {
    generatePredictions();
  }, [predictionDays]);

  // Auto-generate historical data when year changes (exact copy from simulation)
  useEffect(() => {
    generateHistoricalData(historicalYear);
  }, [historicalYear]);

  // Prepare data for charts - exact copy from simulation
  ...
  // Route breakdown data for pie chart - exact copy from simulation
  ...
  // Combined historical and predicted data for trend analysis - exact copy from simulation
  ...
  // Train type breakdown - exact copy from simulation
  ...
  // Load historical data for managerial report
  useEffect(() => {
    const generateHistoricalData = () => {
      ...
      for (let year = 2016; year <= 2025; year++) {
        ...
        // Base demand with year-over-year growth
        const baseGrowth = Math.pow(1.08, year - 2016); // 8% annual growth
        const baseBookings = Math.floor(450000 * baseGrowth);
        
        // Peak during Lebaran and Idul Adha
        const lebaranPeak = Math.floor(baseBookings * 1.4);
        const idulAdhaPeak = Math.floor(baseBookings * 1.25);
        
        data.push({
          year: year,
          bookings: baseBookings,
          lebaranPeak: lebaranPeak,
          idulAdhaPeak: idulAdhaPeak,
          capacity: Math.floor(baseBookings * 1.1), // 110% capacity available
          utilizationRate: Math.min(95, (baseBookings / (baseBookings * 1.1)) * 100)
        });
      }
      
      setHistoricalReportData(data);
    };
    
    generateHistoricalData();
  }, []);

  const [historicalReportData, setHistoricalReportData] = useState([]);

  const generateDemandMatrix = () => {
    const routes = ['Yogyakarta', 'Bandung', 'Surabaya', 'Semarang', 'Malang'];
    const matrix = [];
    ...
    for (let i = 0; i < 7; i++) {
      ...
      routes.forEach(route => {
        const baseCapacity = route === 'Yogyakarta' ? 2800 : route === 'Bandung' ? 3200 : 3000;
        const demandMultiplier = Math.random() * 0.4 + 0.7; // 0.7 to 1.1
        const weekendBoost = date.getDay() === 0 || date.getDay() === 6 ? 1.3 : 1.0;
        
        const predicted = Math.floor(baseCapacity * demandMultiplier * weekendBoost);
        const capacity = Math.floor(predicted / demandMultiplier * 100);
        const utilizationPct = Math.min(120, Math.floor((predicted / baseCapacity) * 100));
        
        matrix.push({
          date: date.toLocaleDateString('id-ID', { weekday: 'short', month: 'numeric', day: 'numeric' }),
          route,
          predicted,
          capacity: baseCapacity,
          utilizationPct,
          queueNeeded: utilizationPct > 85,
          priority: utilizationPct > 100 ? 'critical' : utilizationPct > 85 ? 'warning' : 'normal'
        });
      });
    }
  ...
  // EXACT PREDICT TAB CONTENT FROM SIMULATION
                ...
                <>
                  ...
                  <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div className="bg-gray-50 p-3 rounded-lg">
                      ...
                      <p className="text-lg font-semibold text-gray-900">
                        {yearlyHistoricalData.summary.total_bookings.toLocaleString()}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <p className="text-gray-600">Daily Average</p>
                      <p className="text-lg font-semibold text-gray-900">
                        {yearlyHistoricalData.summary.avg_daily.toLocaleString()}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <p className="text-gray-600">Lebaran {historicalYear}</p>
                      <p className="text-lg font-semibold text-purple-600">
                        {new Date(yearlyHistoricalData.lebaran_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <p className="text-gray-600">Idul Adha {historicalYear}</p>
                      <p className="text-lg font-semibold text-purple-600">
                        {new Date(yearlyHistoricalData.idul_adha_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                      </p>
                    </div>
                  </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {demandMatrix.slice(0, 10).map((item, index) => (
  ...
  const demandMatrix = generateDemandMatrix();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-[#F27500]">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">
          Train Demand Prediction Dashboard
        </h1>
        <p className="text-gray-600 text-sm">
          Enhanced ML Model • 81.26% Accuracy • Islamic Calendar Aware • Last updated: {new Date().toLocaleTimeString('id-ID')}
        </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-600">Peak Day Demand</p>
                    <p className="text-lg font-semibold text-[#F27500]">
                      {prediction.peak_demand?.toLocaleString() || 'N/A'}
                    </p>
                  </div>
                </div>
              </div>
